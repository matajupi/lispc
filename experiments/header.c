...
ObjectMarkPair objects[MAX_NUM_OBJECTS];
size_t op;
...
// =============== GC ===============
typedef struct ObjectMarkPair ObjectMarkPair;
struct ObjectMarkPair
{
    void *object;
    bool mark;
};

void addObject(void *address)
{
    objects[op++] = address;
    if (op >= MAX_NUM_OBJECTS)
        GCRun();
}

void GCRun(Environment *env)
{
    if (env == NULL) {
        return;
    }
...
void *galloc(size_t allocSize, Environment *env)
{
    if (op >= MAX_NUM_OBJECTS)
        GCRun(env);
    void *address = calloc(1, sizeof(allocSize));
    return address;
}
